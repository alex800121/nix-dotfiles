{
  flake.nixosModules.initrd-ssh = { config, lib, ... }: {
    ...
    boot.initrd.network = {
      ...
      ssh.authorizedKeys =
        config.users.users."${config.initConfig.defaultUser}".openssh.authorizedKeys.keys;
    };

    boot.initrd.secrets = {
      "/etc/secrets/initrd_ssh_host_ed25519_key" =
        ../../secrets/initrd_ssh_host_ed25519_key-${config.networking.hostName};
    };
